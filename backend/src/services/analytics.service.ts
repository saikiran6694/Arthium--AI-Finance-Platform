...
export const summaryAnalyticsService = async (userId: string, preset?: DateRangePreset, customFrom?: Date, customTo?: Date) => {
  ...
  if (from && to && rangeValue !== DateRangeEnum.ALL_TIME) {
    const period = differenceInDays(from, to) + 1;

    const isYearly = [DateRangeEnum.LAST_YEAR, DateRangeEnum.THIS_YEAR];
    ...
    const [previous] = await TransactionModel.aggregate(prevPeriodPipline);
  ...
  const result = await TransactionModel.aggregate([
    ...
    //Group the transaction by date (YYYY-MM-DD)
    {
      $group: {
        ...
        incomeCount: {
          $sum: {
            $cond: [{ $eq: ["$type", TransactionEnumType.INCOME] }, 1, 0],
          },
        },
        
        expenseCount: {
          $sum: {
            $cond: [{ $eq: ["$type", TransactionEnumType.EXPENSE] }, 1, 0],
          },
        },
      },
    },
...
export const expensePieChartBreakdownService = async (userId: string, dateRangePreset?: DateRangePreset, customFrom?: Date, customTo?: Date) => {
  const range = getDateRange(dateRangePreset, customFrom, customTo);
  const { from, to, value: rangeValue } = range;

  const filter: any = {
    userId: new mongoose.Types.ObjectId(userId),
    type: TransactionEnumType.EXPENSE,
    ...(from &&
      to && {
        date: {
          $gte: from,
              ...
              percentage: {
                $cond: [
                  ...
                  0,
                  {
                    $round: [
                      {
                        $multiply: [{ $divide: ["$$cat.value", "$totalSpent"] }, 100],
                      },
                      0,
                    ],
                  },
